...
export async function upsertAssessment(formData) {
  ...
  for (const key in assessment) {
    ...
    if (!isNaN(assessment[key]) && assessment[key] !== "") {
      // For id field, ensure it's converted to BigInt
      if (key === 'id') {
        assessment[key] = BigInt(assessment[key]);
      } else {
  ...
  try {
    // Check if this is an update (valid id exists)
    if (assessment.id && assessment.id !== "" && assessment.id !== "0") {
      // For update, we omitted validation to keep it simple for now
      await assessmentRepo.updateAssessment(assessment);
    } else {
      // For new assessments, remove any id field
